...
import com.bctools.auth.entity.UserGroup;
import com.bctools.auth.entity.UserTenant;
import com.bctools.auth.service.UserGroupService;
    ...
    public R<Page<UserVo>> users(@RequestParam(value = "groupId", required = false) String groupId, Page<UserTenant> page) {
        if (ObjectNull.isNotNull(groupId)) {
            ...
            if (ObjectUtils.isNotEmpty(groupUserIds)) {
                userTenantService.page(page, Wrappers.<UserTenant>lambdaQuery().in(UserTenant::getUserId, groupUserIds));
            }
        } else {
